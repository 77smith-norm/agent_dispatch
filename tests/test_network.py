...
def test_dispatch_request_marks_failed_on_invalid_json_response() -> None:
    def handler(_: httpx.Request) -> httpx.Response:
    ...
    async def scenario(db: DispatchDB, request: DispatchRequest) -> None:
        ...
        assert str(exc_info.value) == "endpoint returned invalid JSON with status 200"
    ...
    with TemporaryDirectory() as tempdir:
        ...
        assert (
            stored[0].error_message
            == "endpoint returned invalid JSON with status 200"
        )
